...
@Scope("prototype")
@SpringComponent
public class VisitCreateDialog extends Dialog implements VisitCreateViewInterface {
	...
	VisitCreateDialog(VisitCreatePresenter presenter) {
		...
		final VerticalLayout previousVisitsContainer =
			new VerticalLayout(new H3(getTranslation("previousVisits")),
				previousVisitsGrid);
		previousVisitsContainer.setPadding(false);
		setHeaderTitle(getTranslation("newVisit"));
		...
		add(new H3(getTranslation("pet")), petForm,
			visitForm,
			previousVisitsContainer);
		nameTextField.focus();
		getFooter().add(new Button("Cancel", e -> close()), addVisitButton);
	}
